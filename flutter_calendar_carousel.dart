...
import 'package:african_puzzle_work/core/constants.dart';
import 'package:african_puzzle_work/features/presentation/widgets/flutter_calendar_carousel/src/calendar_header.dart';
...
class CalendarCarousel<T extends EventInterface> extends StatefulWidget {
  ...
  CalendarCarousel({
    ...
    this.weekDayFormat = WeekdayFormat.weekdays,
...
class _CalendarState<T extends EventInterface>
  ...
  Widget renderDay(
    ...
    // Shape conditions
    final hasCustomShapeBorder = widget.markedDateCustomShapeBorder != null &&
        markedDatesMap != null &&
        markedDatesMap.getEvents(now).length > 0;
    final hasCircularBorder = widget.daysHaveCircularBorder ?? false;

// Colors
    final borderColor = isSelectedDay
    ...
    final backgroundColor = isSelectedDay
...
// Shape
    final shape = hasCustomShapeBorder
...
//            : DateTime.now();
  ...
  TextStyle? getDayStyle(
    ...
    bool isMultipleMarked = widget.multipleMarkedDates?.isMarked(now) ?? false;
    TextStyle? multipleMarkedTextStyle =
        widget.multipleMarkedDates?.getTextStyle(now);

    // Weekend condition
    bool isWeekend = _localeDate.dateSymbols.WEEKENDRANGE
            .contains((index - 1 + firstDayOfWeek) % 7) &&
        !isSelectedDay &&
        isThisMonthDay &&
        !isToday;
    ...
    TextStyle? weekendStyle = isSelectable
        ? widget.weekendTextStyle
        : widget.inactiveWeekendTextStyle;
    TextStyle? inactiveDaysStyle = widget.inactiveDaysTextStyle;
    TextStyle? prevDaysStyle = widget.prevDaysTextStyle;
    TextStyle? nextDaysStyle = widget.nextDaysTextStyle;
    TextStyle? todayStyle = const TextStyle(
        fontSize: 18, fontWeight: FontWeight.bold, color: kPurpleDark);
    TextStyle? daysTextStyle = widget.daysTextStyle;

    return selectedDayStyle ??
        (isMultipleMarked
            ? multipleMarkedTextStyle
            : (isWeekend
                ? weekendStyle
                : (!isSelectable
                    ? inactiveDaysStyle
                    : (isPrevMonthDay
                        ...
                        : (isNextMonthDay
                            ? nextDaysStyle
                            : (isToday ? todayStyle : daysTextStyle))))));
  }
